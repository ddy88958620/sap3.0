			...
			<table>
	       		<tr>
	       			...
	       			<td>
	       				&nbsp;&nbsp;&nbsp;
	       				<a>
	       					<button type="button" id="sync" class="btn btn-info btn-sm" onClick="syncHotword()">
           						<span class="ace-icon fa fa-search icon-on-right bigger-110"></span>&nbsp;同步
           					</button>
	       				</a>
	       			</td>
	   			</tr>
	...
	<!-- 关键词添加modal -->
 	<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-top:-100px;margin-left:-200px; left: 0% ">
   		<div class="modal-dialog">
      		<div class="modal-content" style="width:550px;height:300px;" id="modalContent">
         		...
         		<div class="modal-body" id="modal-body" align="center" style="margin-top:40px" >
        			<table id="addTable" class="addTable" style="width:90%">
		                <tr>
		                 ...
		                 <tr>
		                  <td style="padding-left:0px;margin-left:0px">关键词内容:</td>
		                  <td>
		                  	<!-- <input id="addContent" style="width:100%; height:80px; border:0px;"></input> -->
		                  	<input id="addContent" style="width:100%" class="input" type="text"/>
		                  </td>
		                </tr>
            		</table>
         		</div>
            	...
            	<div  align="right" style=" padding-right:42px; padding-bottom:22px;">
		            <button id="addButton" onclick="addHotword()" type="button" class="btn btn-info btn-sm" style="margin-right:10px"><i class="ace-icon fa fa-check bigger-110"></i>确定</button> 
	 ...
	 <div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-top:-100px;margin-left:-200px">
	 	<div class="modal-dialog">
	    	<div class="modal-content" style="width:550px;height:450px;" id="modalContent1">
	        	<div class="modal-header">
		            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
		                  &times;
		            </button>
	         	...
	         	<div class="modal-body" id="modal-body" align="center" style="margin-top:40px">
	        		<table id="updateTable" class="addTable" style="width:90%">
		                <tr>
		                 ...
		                 <tr>
		                  <td style="padding-left:0px;width:70px">关键词内容:</td>
		                  <td>
		                  	<input id="updateContent" style="width:100%" class="input" type="text"/>
		                  </td>
		                </tr>
	            	</table>
	       		...
	       		<hr style="color:#d0d0d0;margin-bottom:15px;margin-top: 0"/>
	            <div align="right" style=" padding-right: 42px;padding-bottom: 22px;">
		            <button id="updateButton" onclick="updateHotword()" type="button" class="btn btn-info btn-sm" style="margin-right:10px"><i class="ace-icon fa fa-check bigger-110"></i>确定</button> 
		            <button type="reset" class="btn btn-sm" data-dismiss="modal" ><i class="ace-icon fa fa-undo bigger-110" ></i>取消</button>
		        </div>
	      	</div>
	    </div>
	 	...
	 	<div class="modal-dialog">
	    	<div class="modal-content">
	        	...
	        	<hr style="margin-bottom: 10px;"/>
			  	<div align="right" style="padding-right: 42px; padding-bottom: 22px;">
					<button id="deleteButton" onclick="deleteHotword()" type="button" class="btn btn-info btn-sm" style=" padding-right: 10px "><i class="ace-icon fa fa-check bigger-110"></i>确定</button> 
		            <button type="reset" class="btn btn-sm" data-dismiss="modal"><i class="ace-icon fa fa-undo bigger-110"></i>取消</button>
		        </div>
      		</div>
		</div>
	...
	<div class="modal fade" id="errorMsg" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" >
	   <div class="modal-dialog">
	      <div class="modal-content" style="width:460px;height:300px;margin-left:70px;">
	         ...
	         <div class="modal-body" id="modal-msg" align="center"  style="margin-top:40px">
...
function addSearchInfo(aoData) {
	aoData.push({"name":"content","value":queryData.val()});
}
...
function getAllRecord() {
	...
	nowDataTable = $('#dataTable').dataTable({
		...
		"order": [[ 0, 'asc' ]],
		"sAjaxSource": "../hotword/dataGrid",	//请求内容为退一步请求的内容
		"aoColumns":aoColumns,
...
function editFun(uuid){
	...
	$.post("../hotword/loadById",{uuid:uuid},function(result){
		var hotword = result.obj;
		var html ="";
		$("#searchConditionList1").html('');
		searchIdArray = new Array();
		var arr=eval(hotword.searchInfo);
	if (typeof(arr)!='undefined'&&arr!=null){
		...
		$("#updateUuid").val(hotword.uuid);
		$("#updateState").val(hotword.state);
		$("#updateContent").val(hotword.content);
...
/** -- 同步关键词 -- */
function syncHotword() {
	loadmask();
	$.post("../hotword/sync",{},function(result){
		disloadmask();
		var success = result.success;
		if(success) {
			Modal_Alert('关键词管理', result.msg);
			getAllRecord();
		} else {
			Modal_Alert('关键词管理', result.msg);
		}
		...
		disloadmask();
		Modal_Alert('关键词管理','通讯失败，请重新发起请求！');
	});
}
...
function addHotword() {
	...
	loadmask();
	$.post("../hotword/add", data, function(result){
		disloadmask();
		var success = result.success;
		if(success) {
			$("#addModal").modal("hide");
...
function updateHotword() {
	...
	loadmask();
	$.post("../hotword/edit", data, function(result){
		disloadmask();
		var success = result.success;
		if(success) {
...
function validation(data) {
	if(data.content == null || data.content == '') {
		Modal_Alert('关键词管理', "请填写关键词内容");
		return false;
	}
	if(data.content.length > 64) {
		Modal_Alert('关键词管理', "关键词内容过长");
		return false;
	}
	return true;
}
