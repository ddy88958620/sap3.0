...
<div id="notVisiable" style="display:none"></div>
	...
	<div style="position:absolute;top:0px;bottom:0px;left:0px;right:0px;height:100%;overflow-x:hidden;overflow-y:hidden;padding:10px;" >
		...
		<!-- <div id="nowPosition" style="position:absolute;right:5px;top:15px;">
			<span class="label label-xlg label-yellow arrowed-in">当前位置： 质检规则 - 
				<font  id="tabSelect">自动质检规则集管理</font>
			</span>
		</div> -->
	...
	<!-- 添加规则集modal -->
 	<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-top:-100px; left: 0% ">
   		<div class="modal-dialog">
      		<div class="modal-content" style="width:660px;height:360px;" id="modalContent">
         		...
         		<div class="modal-body" id="modal-body" align="center" >
        			<table id="addTable" class="addTable">
                		<tr>
		                  ...
		                  <td><input id="addName" name="addName" placeholder="请输入规则集名称" class="input" type="text"/></td>
		                </tr>
		                <tr>
            		...
            		<div id="searchList" style="width:70%; height:250px; margin-top:5px; margin-left:90px;overflow-y:auto; overflow-x:auto;display:none">
            	...
            	<div  align="right" style=" padding-right:42px; padding-bottom:22px;">
		            <button id="addButton" onclick="addAutoRules()" type="button" class="btn btn-info btn-sm" style="margin-right:10px"><i class="ace-icon fa fa-check bigger-110"></i>确定</button> 
	 ...
	 <div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-top:-100px;margin-left:-200px">
	 	<div class="modal-dialog">
	    	<div class="modal-content" style="width:660px;height:360px;" id="modalContent1">
	        	<div class="modal-header">
		            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
		                  &times;
		            </button>
	         	...
	         	<div class="modal-body" id="modal-body" align="center" >
	        		<table id="updateTable" class="addTable">
		                <tr>
		                  <td style="padding-left:65px">名称:</td>
		                  <td><input id="updateName" name="updateName"  placeholder="请输入规则集名称" class="input" type="text"/></td>
		                </tr>
		                <tr>
		                  <td style="padding-left:65px">状态:</td>
		                ...
		                <tr>
		                 ...
		                 <tr>
		                  <td style="padding-left:17px">规则集条件:</td>
		                  <td >
		                  	<button type="button" class="btn btn-info btn-sm" onclick="updateSearchCondition()" style="width:95%; height:25px; border:0px;">
			            	<font style="font-size:13px;">
			            		添加
			            	</font>
		           	 	</button>
	            	...
	            	<div id="searchList1" style="width:70%; height:250px; margin-top:5px; margin-left:90px;overflow-y:auto; overflow-x:auto;display:none">
					<table style="width:100%;">
	       		...
	       		<hr style="color:#d0d0d0;margin-bottom:15px;margin-top: 0"/>
	            <div align="right" style=" padding-right: 42px;padding-bottom: 22px;">
		            <button id="updateButton" onclick="updateAutoRules()" type="button" class="btn btn-info btn-sm" style="margin-right:10px"><i class="ace-icon fa fa-check bigger-110"></i>确定</button> 
		            <button type="reset" class="btn btn-sm" data-dismiss="modal" ><i class="ace-icon fa fa-undo bigger-110" ></i>取消</button>
		        </div>
	      	</div>
	    </div>
	 	...
	 	<div class="modal-dialog">
	    	<div class="modal-content">
	        	...
	        	<hr style="margin-bottom: 10px;"/>
			  	<div align="right" style="padding-right: 42px; padding-bottom: 22px;">
					<button id="deleteButton" onclick="deleteAutoRules()" type="button" class="btn btn-info btn-sm" style=" padding-right: 10px "><i class="ace-icon fa fa-check bigger-110"></i>确定</button> 
		            <button type="reset" class="btn btn-sm" data-dismiss="modal"><i class="ace-icon fa fa-undo bigger-110"></i>取消</button>
		        </div>
      		</div>
		</div>
...
function getAllRecord() {
	...
	nowDataTable = $('#dataTable').dataTable({
		...
		"order": [[ 0, 'asc' ]],
		"sAjaxSource": "../autoRules/dataGrid",	//请求内容为退一步请求的内容
		"aoColumns":aoColumns,
...
function editFun(uuid){
	...
	$.post("../autoRules/loadById",{uuid:uuid},function(result){
		var autoRules = result.obj;
		var html ="";
		$("#searchConditionList1").html('');
		searchIdArray = new Array();
		var arr=eval(autoRules.searchInfo);
		if (typeof(arr)!='undefined'&&arr!=null){
				...
				if(arr.length>0){
					$("#modalContent1").css("height","600px");
		...
		$("#updateName").val(autoRules.name);
		$("#updateRemark").val(autoRules.remark);
		$("#updateUuid").val(autoRules.uuid);
		$("#updateState").val(autoRules.state);
...
function addAutoRules() {
	...
	if(searchInfo==null||"null"==searchInfo){
		return ;
	}
	if(!validation(data)) {
		return;
	...
	loadmask();
	$.post("../autoRules/add", data, function(result){
		disloadmask();
...
function updateAutoRules() {
	...
	if(data.uuid == null || data.uuid == '') {
		Modal_Alert('自动质检规则集管理', "信息缺失，请重新操作！");
		return;
	}
	if(!validation(data)) {
		return;
	...
	loadmask();
	$.post("../autoRules/edit", data, function(result){
		disloadmask();
		var success = result.success;
		if(success) {
...
/** -- 删除规则集类型 -- */
function deleteAutoRules() {
	$("#deleteModal").modal("hide");
	var uuid = $("#deleteUuid").val();
	
	loadmask();
	$.post("../autoRules/delete", {uuid:uuid}, function(result){
		disloadmask();
		var success = result.success;
		if(success) {
			getAllRecord();
...
function validation(data) {
	...
	if(data.name == null || data.name == "") {
		Modal_Alert('自动质检规则集管理', "请填写规则集名称");
		return false;
	}
	if(data.name.length > 64) {
		Modal_Alert('自动质检规则集管理', "规则集名称过长");
		return false;
	}
	var defaultValue=$.trim($("#addRemark").attr('placeholder'));
	if(defaultValue==data.remark){
		data.remark="";
	}
	if(data.remark == null || data.remark == "") {
		Modal_Alert('自动质检规则集管理', "请填写规则集备注");
		return false;
	}
	if(data.remark.length > 128) {
		Modal_Alert('自动质检规则集管理', "规则集备注过长");
		return false;
	}
	return true;
}
...
function changeValueWidget(value) {
	...
	if(obj.type == '0') {
		...
		$("#fieldLabel" + value).html("字段取值：");
		html += '<div class="tags" style="border-color:#b3d4e7; overflow-y:hidden; overflow-x:hidden; width:100%;">' + 
					...
					<div class="widget-main">
						<div>
							...
							<select class="input-medium" id="searchField{id}" name="searchField{id}" 
									style="color:#88878B; border-color:#b3d4e7; width:100%; height:30px;font-size:12px;" onchange="changeValueWidget('{id}')">
							</select>
						</div>
						<br />
						<div>
