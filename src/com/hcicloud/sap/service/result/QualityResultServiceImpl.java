...
@Service
public class QualityResultServiceImpl implements QualityRusultServiceI {
	@Autowired
	...
	private String getSearchString(String uuid, String qualityStatus, PageFilter page,String startTime,String endTime,String userGroupId) {
		 ...
		 * 如果是质检员(roleId为2),则可以看到自己质检的结果；
		 * 如果是坐席(roleId为3),则可以看到自己被质检的结果
		 */
		String roleCode = getRoleCode(uuid);
		if(roleCode.indexOf("1")!=-1){
			shouldBuilder.append(getTermString("userGroupId", userGroupId));
		}
		if(roleCode.indexOf("2")!=-1){
			shouldBuilder.append(getTermString("manualData.inspectorId", uuid));
		}
		if(roleCode.indexOf("3")!=-1){
			shouldBuilder.append(getTermString("relateData.userId", uuid));
		}
		
		//处理should字符串
		if(shouldBuilder.charAt(shouldBuilder.length() - 1) == ',') {
			shouldBuilder.deleteCharAt(shouldBuilder.length() - 1);
		}
		shouldBuilder.append("],");
		
		searchBuilder.append(shouldBuilder.toString());
