...
public class AnnoyController {
	...
	@RequestMapping("/downloadTransTxt")
	@ResponseBody
	public String downloadTransTxt(AnnoyModel annoyModel,HttpServletResponse response,HttpServletRequest request){
		try {
			//解决get乱码
			annoyModel.setAnnoyType(new String(annoyModel.getAnnoyType().getBytes("iso8859-1"),"utf-8"));
			annoyModel.setSeatAttitude(new String(annoyModel.getSeatAttitude().getBytes("iso8859-1"),"utf-8"));
    		...
    		File file = annoyService.getAnnoyFile(annoyModel,new PageFilter(0,(int)total));
    	     if (file!=null) {
