...
public class AntZipUtil {
	...
	/**
	 * 这里将file文件转化成zip文档，当前页面下载
	 * @param file
	 * @param response
	 * @param isDelete
	 * @param request
	 */
	public static void downloadFile(File file, HttpServletResponse response, boolean isDelete, HttpServletRequest request) {
		try {
			// 以流的形式下载文件。
			BufferedInputStream fis = new BufferedInputStream(new FileInputStream(file.getPath()));
			//返回的字节数，保证从此输入流中读取而不阻塞
	...
	/**
	 * 这个是提取录音文件中转译文本导出的模块，没有对比有什么不同
	 * @param files
	 * @param outputStream
	 * @throws Exception
	 */
	// TODO 对比和上述的filesTOZip 方法有什么不同
	...
	public static void main(String[] temp) {
		...
		// 压缩多个指定的文件 到ZIP
		System.out.println(AntZipUtil.zip(baseDirName, fileNames, zipFileName,
				"GBK"));
