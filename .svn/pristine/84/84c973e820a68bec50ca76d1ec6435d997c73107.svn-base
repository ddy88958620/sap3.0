		...
		<div id="cesi" style="position:absolute;width:100%;left:30px;">
			<table>
       			<tr>
					...
					<td>
						<select id="qualityDataSelect" class="select">
...
var aoColumns = [
					{  
				    	"mDataProp" : "key",  "sTitle" : "质检点","bVisible": true,"sDefaultContent" : "未违规","sWidth": "25%","sClass" : "center",
                    ...
                    {  
 	                   "mDataProp" : "violation",   "sTitle" : "违规率%","sWidth": "10%","bVisible": true,"sDefaultContent" : "",  "sClass" : "center",
						"render": function ( data, type, full, meta ) {
...
function getAllRecord(){
	...
	nowDataTable = $('#dataTable').dataTable({
		...
		"order": [[ 0, 'asc' ]],
		"sAjaxSource": "${ctx }/success/report",//请求内容为退一步请求的内容
		"aoColumns":aoColumns,
...
function addSearchInfo(aoData) {
	...
	aoData.push({"name": "autoRules", "value": $("#qualityDataSelect option:selected").text()});
}
